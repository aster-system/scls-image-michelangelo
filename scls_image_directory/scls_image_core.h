...
namespace scls
{
	...
	public:
        ...
        inline std::shared_ptr<Image> resize_adaptative(unsigned short new_width, unsigned short new_height) {
            std::shared_ptr<Image> current_image = resize_adaptative_width(new_width);
            if(current_image.get() == 0) return resize_adaptative_height(new_height);
            return current_image.get()->resize_adaptative_height(new_height); };
        // Returns a shared ptr of the image with a new height, adaptated
        inline std::shared_ptr<Image> resize_adaptative_height(unsigned short new_height) {
        ...
        inline std::shared_ptr<Image> resize_adaptative_width(unsigned short new_width) {
            if(new_width < width()) {
                // Create the new image
